...
export const CategoryForm: React.FC<CategoryFormProps> = ({
    ...
    const toastMessage = initialData ? "Catégorie mise à jour" : "Catégorie crée"
    ...
    const onDelete = async () => {
        try{
            ...
            router.refresh();
            router.push(`/${params.storeId}/categories`);
            toast.success("Bravo! Catégorie supprimée")
        } catch(erro){
            toast.error("Supprimez d'abord tout les produits liés à cette catégorie.")
        } finally{
            setLoading(false)
            setOpen(false)
        }
        ...
        <>
          ...
          <div className="md:grid md:grid-cols-3 gap-8">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Panneaux d&apos;affichage</FormLabel>
                  <Select 
                  disabled={loading} 
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue 
                          defaultValue={field.value} 
                          placeholder="Choisir un panneau d'affichage"
                        />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
